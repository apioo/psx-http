 ...
 * @link    http://phpsx.org
 */
class DigestAuthentication implements FilterInterface
{
    /**
     * @var \Closure
     */
    ...
    /**
     * @var \PSX\Http\Filter\DigestAuthentication\StoreInterface
     */
    ...
    /**
     * The ha1Callback should return "md5([username]:[realm]:[pw])" which 
     * is then used to compare the response from the client. If its successful 
     * the onSuccess callback is called else the onFailure. If the Authorization
     ...
     * @param \PSX\Http\Filter\DigestAuthentication\StoreInterface $digestStore
     */
    public function __construct(Closure $ha1Callback, StoreInterface $digestStore)
    {
        ...
        $this->onMissing(function (ResponseInterface $response) use ($digestStore) {
            $digest = new Digest();
            $digest->setNonce($this->generateNonce());
            $digest->setOpaque($this->generateOpaque());

            $digestStore->save($digest->getOpaque(), $digest);
    ...
    public function handle(RequestInterface $request, ResponseInterface $response, FilterChainInterface $filterChain)
    {
        ...
        if (!empty($authorization)) {
            ...
            if ($type == 'Digest' && !empty($data)) {
                ...
                $digest = $this->digestStore->load($params['opaque']);

                if (!$digest instanceof Digest) {
                    throw new BadRequestException('Digest not available');
                }

                if ($digest->getOpaque() != $params['opaque']) {
                ...
                if ($algo == 'MD5-sess') {
                    $ha1 = md5($ha1 . ':' . $digest->getNonce() . ':' . $params['cnonce']);
                }
                ...
                if ($qop == 'auth' || $qop == 'auth-int') {
                    $hash = md5($ha1 . ':' . $digest->getNonce() . ':' . $params['nc'] . ':' . $params['cnonce'] . ':' . $qop . ':' . $ha2);
                } else {
                    $hash = md5($ha1 . ':' . $digest->getNonce() . ':' . $ha2);
                }
    ...
    protected function generateOpaque()
    {
        return sha1(time() . uniqid());
    }
}
